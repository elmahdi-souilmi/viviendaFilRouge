...
foreach ($Students as $Student) {?>
					<tr>
						<td> <?php echo '<img src="' . $Student->Picture . '" width="140%" />'; ?> </td>
						<td> <?php echo $firstNamee = $Student->FirstName; ?></td>
						<td> <?php echo $LastNamee = $Student->LastName; ?></td>
						<td> <?php echo $phonee = $Student->Phone; ?></td>
						<td> <?php echo $usernamee = $Student->UserName; ?></td>
						<td> <?php echo $passe = $Student->Password; ?> </td>
						<td> <?php echo $emaile = $Student->Email; ?> </td>
						<td> <?php echo $Adresse = $Student->Adress; ?></td>
						<td> <?php echo $citye = $Student->city; ?> </td>
						<?php if ($Student->NeedRoomMate == 1) {?>
							<td> YES </td>
						<?php } else {?>
							<td> NO </td>
						<?php }?>

						<input type="hidden" id="custId" name="custId" value="<?php echo $id = $Student->IdUser; ?>">
						<td>
						<button style="margin-bottom: 4px;" type="button"   onClick="test('<?php echo $firstNamee ?>','<?php echo $LastNamee ?>', '<?php echo $phonee ?>', '<?php echo $usernamee ?>', '<?php echo $passe ?>', '<?php echo $emaile ?>', '<?php echo $Adresse ?>', '<?php echo $citye ?>', '<?php echo $Student->NeedRoomMate ?>','<?php echo $id ?>','<?php echo $Student->Picture ?>')"  class="btn btn-outline-primary btn-rounded waves-effect" data-backdrop="static" data-toggle="modal" data-target="#editEmployeeModal"><i class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i></button>
		...
		<div class="modal-content">
			<form  action="Student.php" method="POST" enctype="multipart/form-data" >
				...
				<div class="form-group">
					...
					<div class="form-group">
						<input type="hidden" id="hiden" name="hiden" value="<?php echo $id = $Student->IdUser; ?>">
					</div>
				</div>
				<div class="modal-footer">
					<input type="button" class="btn btn-default" data-dismiss="modal" value="Cancel">
...
<script>
function test(firstname, lastname, phone, username, pass, email, adress, city,mate, id, picture){
    console.log(firstname, lastname, phone, username, pass, email, adress, city,mate, id, picture );
